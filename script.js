...
enterBtn.addEventListener("click", () => {
  gateScreen.style.transition = "opacity 0.8s ease";
...
function renderGraves() {
  ...
  graves.forEach((grave) => {
    ...
    card.innerHTML = `
      ...
      <div class="tombstone-dates">${grave.born || "?"} — ${grave.died || "?"}</div>
...
graveForm.addEventListener("submit", (e) => {
  ...
  const newGrave = {
    ...
    born: document.getElementById("pBorn").value,
...
function openViewModal(id) {
  ...
  viewContent.innerHTML = `
    ...
    <div class="dates">${grave.born || "?"} — ${grave.died || "?"}</div>
